...
def get_all_topics(index_url, category_slug):
    """Scrapes the main index page to find every topic folder dynamically."""
    print(f"Scanning directory: {index_url} ...")
    ...
    for a in soup.find_all('a', href=True):
        ...
        # Break the URL into parts, ignoring trailing or double slashes
        parts = [p for p in href.split('/') if p]
        
        # DYNAMIC FILTER: Looks for the specific category slug (e.g., 'logical-reasoning')
        if len(parts) >= 2 and parts[-2] == category_slug and parts[-1] != 'questions-and-answers':
            topic_name = a.get_text(strip=True)
            
            # Ignore empty links or pure numbers
            if len(topic_name) > 2 and not topic_name.isdigit():
                topic_slug = parts[-1]
                full_url = f"https://www.indiabix.com/{category_slug}/{topic_slug}/"
...
def scrape_topic_to_pdf(start_url, output_filename, topic_name, category_name):
    ...
    pdf.cell(0, 10, txt=f"{category_name}: {topic_name}", ln=True, align='C')
    pdf.ln(5)
...
# =====================================================================
# THE MULTI-CATEGORY MASTER LAUNCHER
# =====================================================================
if __name__ == "__main__":
    
    # We define the categories we want to scrape in a list of dictionaries
    ...
    for category in CATEGORIES_TO_SCRAPE:
        print(f"\n\n=======================================================")
        print(f"🚀 INITIATING MASTER SCRAPE FOR: {category['name']}")
        print(f"=======================================================\n")
        
        all_topics = get_all_topics(category['url'], category['slug'])
        print(f"🎯 Found {len(all_topics)} different topics in {category['name']}!\n")
        
        if len(all_topics) == 0:
            print(f"Wait, 0 topics found for {category['name']}. Skipping...")
            continue
            
        if not os.path.exists(category['folder']):
            os.makedirs(category['folder'])
            
        for topic in all_topics:
            file_path = os.path.join(category['folder'], sanitize_filename(topic['name']))
            
            if os.path.exists(file_path):
                print(f"⏩ Skipping '{topic['name']}' -> PDF already exists.")
                continue
                
            scrape_topic_to_pdf(topic['url'], file_path, topic['name'], category['name'])
